...
如果要做系统化动态难度调整，参考 [关卡类游戏 DDA 实验框架](./dda-level-framework.md)。GameAlgo 建议把 DDA 作为实验系统上的标准用法：游戏提供 Adapter 和少量可调 knobs，平台通过实验比较不同 knobs 组合的逐日标准化用户价值和 LTV proxy，同时保留实际收入、实际 ARPU、新用户留存与实际综合 CPM 作为诊断数据。
