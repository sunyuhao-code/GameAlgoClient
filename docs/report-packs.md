...
| `Experiments` / 实验总览 | 多个实验的关键指标横向对比、实验版本复盘 | 不建议把所有实验图都堆在这里；优先在业务 tab 的 group 内加 Strategy selector |
...
- `line`：时间趋势，例如 DAU、收入、留存率、ARPU、模式渗透率。
- `bar`：离散维度对比或分布，例如最大关卡分布、关卡流失、广告位收入、模式人数。分桶类柱状图要配置 `sort`，例如 `bucket_order`。
- `pie`：少量分类占比，例如广告位收入占比、广告类型曝光占比。分类很多时优先改成 bar/table，或在 SQL 里做 TopN。
- `cohort_matrix`：新用户 cohort 指标矩阵，例如 D0-D14 最大关卡、LTV、累计时长。
- `table`：明细、排查用结果。不要把 table 当成主要分析入口，除非用户确实需要逐行看。

常见拆分方式：

- 如果一个图回答“发生了什么”，放在 `Overview`、`Revenue` 或 `Retention`。
...
| `revenue.placement@1` | 广告变现标准看板。只生成一个 `广告变现` group，依次展示三张全宽的“堆积柱 + 合计线”：按广告位拆分的每日广告收入、按广告类型拆分的用户人均看广告数，以及按广告位拆分的 ARPU（内购作为单独的“内购”系列）。每个广告位 ARPU 的分母都是所选用户类型或实验组的全部活跃用户，而不是看过该广告位的用户。其后展示广告位和广告类型的收入/曝光占比。整个 group 支持用户类型 selector（全部/新用户/老用户）；实验详情页按实验组画总人均指标折线，不堆积互斥实验组。 | 广告 SDK 确认实际产生收入的 `ad_view` 事件，必填 `placement`、`adType`、`revenue`、`currency`，可选 `network` 和 `mode`。广告失败、未填充、播放失败，或广告 SDK 没有确认产生收入的展示，不要上报到 `ad_view`。 |
