...
- 所有实验组使用同一个 `k`。`k` 的上限在创建实验时按参考周期固定为 `min(14, minWindowDays - 3)`，并要求每个实验组在对应 Dx 至少有 10 个成熟进入用户。

新用户留存、新用户 LTV、实际 ARPU 和实际收入作为诊断信息单独展示，不再通过固定倍率间接折算主指标。

### 成熟窗口

托管实验不会为了等待更远的 Dx 拉长周期；延长只用于增加样本、降低随机分流波动。达到参考周期后，平台使用本轮真实用户与线上分流规则模拟随机 seed：证据明确则完成本轮，不明确则逐日延长，直到达到最长周期。整个过程中 `k` 保持不变。
